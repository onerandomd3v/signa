...
- The delivery worker uses `SIGNA_DELIVERY_CONSUMER_GROUP`, `SIGNA_DELIVERY_POLL_INTERVAL`, `SIGNA_DELIVERY_RETRY_MAX_ATTEMPTS`, `SIGNA_DELIVERY_RETRY_BACKOFF`, and `SIGNA_DELIVERY_ATTEMPT_LEASE`. Delivery attempts and outcomes are authoritative in PostgreSQL; the provider adapter receives a stable idempotency key. COD-215 supplies the generic worker boundary; Web Push remains a separate adapter task.
