...
- The AI extraction worker uses `SIGNA_AI_PROVIDER_TIMEOUT`, `SIGNA_AI_RETRY_MAX_ATTEMPTS` (bounded to five), and `SIGNA_AI_RETRY_BACKOFF` to limit provider latency and transient-failure retries. Invalid structured output remains pending for inspection/retry and is never persisted as validated evidence.
