...
- `SIGNA_PUBLIC_INCIDENT_GRID_METERS`, `SIGNA_PUBLIC_INCIDENT_MIN_RADIUS_METERS`, and `SIGNA_PUBLIC_INCIDENT_SIMPLIFY_METERS` configure the required positive-meter `signa.public-incident-geometry.v1` policy used to generalize public incident geometry. The API refuses to start when these values are missing or invalid.
- The AI extraction worker uses `SIGNA_AI_PROVIDER_TIMEOUT`, `SIGNA_AI_RETRY_MAX_ATTEMPTS` (bounded to five), and `SIGNA_AI_RETRY_BACKOFF` to limit provider latency and transient-failure retries. Invalid structured output remains pending for inspection or an explicitly controlled retry and is never persisted as validated evidence.
- The delivery worker boundary uses `SIGNA_DELIVERY_CONSUMER_GROUP`, `SIGNA_DELIVERY_POLL_INTERVAL`, `SIGNA_DELIVERY_RETRY_MAX_ATTEMPTS`, `SIGNA_DELIVERY_RETRY_BACKOFF`, and `SIGNA_DELIVERY_ATTEMPT_LEASE`. Delivery attempts and outcomes are authoritative in PostgreSQL; the provider adapter receives a stable idempotency key. COD-215 supplies the generic boundary, but `cmd/worker` does not consume delivery requests until a real provider adapter is configured; Web Push remains a separate COD-217 task.
- The versioned extraction evaluation suite exposes an opt-in acceptance gate through explicitly injected `minimum_passed` and/or `minimum_pass_rate` values. It has no repository default; owner approval is required before wiring thresholds into CI, so fixture comparisons cannot silently become an unapproved product policy.
...
- The incident worker also requires `SIGNA_INCIDENT_RESOLVING_AFTER`, `SIGNA_INCIDENT_RESOLVED_AFTER`, `SIGNA_INCIDENT_EXPIRED_AFTER`, and `SIGNA_INCIDENT_LIFECYCLE_SWEEP_INTERVAL`; these are positive Go durations, the first three must be strictly increasing, have no product-policy defaults, and are not required by the API process.
