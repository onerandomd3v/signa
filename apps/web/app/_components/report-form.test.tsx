...
import type { DeviceLocation, ReportAcknowledgement } from "../../lib/api/generated";
...
describe("ReportForm", () => {
  ...
  it("submits text and shows the accepted reference", async () => {
    ...
    expect(send.mock.calls[0][0]).toEqual({ raw_text: "Smoke near the station." });
    expect(send.mock.calls[0][1]).toMatch(/^[0-9a-f-]{36}$/i);
    expect(await screen.findByText("Report accepted for processing.")).toBeTruthy();
  ...
  it("requires non-empty text", () => {
    ...
    render(<ReportForm submitReport={send} />);
    submit();
    expect(screen.getByRole("alert").textContent).toBe("Add a short description.");
    expect(screen.getByLabelText(/what happened/i).getAttribute("aria-invalid")).toBe("true");
    expect(send).not.toHaveBeenCalled();
  });

  it("prevents duplicate submissions while the request is pending", async () => {
    let resolve!: (value: typeof success) => void;
    const send = vi.fn(() => new Promise<typeof success>((done) => { resolve = done; }));
    render(<ReportForm submitReport={send} />);
    enterReport();
    submit();
  ...
  it("keeps a key for an unchanged retry and rotates it when the text changes", async () => {
    const send = vi.fn().mockResolvedValueOnce(failure(500)).mockResolvedValueOnce(failure(400)).mockResolvedValueOnce(success);
    render(<ReportForm submitReport={send} />);
    enterReport();
    submit();
  ...
  it("honors Retry-After and retains the idempotency key after rate limiting", async () => {
    const send = vi.fn().mockResolvedValueOnce(failure(429, "15")).mockResolvedValueOnce(success);
    render(<ReportForm submitReport={send} />);
    enterReport();
    submit();
    expect((await screen.findByRole("alert")).textContent).toContain("15 seconds");
    submit();
    await screen.findByText("Report accepted for processing.");
    expect(send.mock.calls[1][1]).toBe(send.mock.calls[0][1]);
  });
  ...
  it("preserves text after a network error and retries with the same key", async () => {
    const send = vi.fn().mockRejectedValueOnce(new Error("offline")).mockResolvedValueOnce(success);
    render(<ReportForm submitReport={send} />);
    enterReport();
    submit();
    await screen.findByRole("alert");
    expect((screen.getByLabelText(/what happened/i) as HTMLTextAreaElement).value).toBe("Smoke near the station.");
    submit();
    await screen.findByText("Report accepted for processing.");
    expect(send.mock.calls[1][1]).toBe(send.mock.calls[0][1]);
  });
  ...
  it("sends opt-in location without displaying coordinates", async () => {
    const location: DeviceLocation = { latitude: 6.5244, longitude: 3.3792, accuracy: 42 };
    const requestLocation = vi.fn().mockResolvedValue(location);
    const send = vi.fn().mockResolvedValue(success);
    render(<ReportForm submitReport={send} requestLocation={requestLocation} />);
    ...
    submit();
    await screen.findByText("Report accepted for processing.");
    expect(send.mock.calls[0][0]).toEqual({ raw_text: "Smoke near the station.", device_location: location });
  });
  ...
  it.each(["denied", "unavailable", "timeout", "low-accuracy"] as const)(
    ...
    async (reason) => {
      const send = vi.fn().mockResolvedValue(success);
      render(<ReportForm submitReport={send} requestLocation={vi.fn().mockRejectedValue(new DeviceLocationError(reason))} />);
      fireEvent.click(screen.getByRole("button", { name: "Share my location" }));
  ...
  it("does not claim a report is unavailable in the default production path", () => {
    render(<ReportForm />);
    expect(screen.queryByText(/not sent|submission isn’t available/i)).toBeNull();
  });
});
