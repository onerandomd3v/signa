...
func (s *PostgresStore) StartAttempt(ctx context.Context, request Request, now time.Time) (StartResult, error) {
	...
	err = tx.QueryRow(ctx, `
		SELECT d.state, d.alert_id::text, d.idempotency_key, d.channel, d.priority, d.payload, d.attempts, d.last_attempt_at, d.next_attempt_at
		FROM deliveries d WHERE d.id = $1 FOR UPDATE`, request.DeliveryID).
		Scan(&state, &storedAlertID, &storedKey, &channel, &priority, &payload, &attempts, &lastAttempt, &nextAttempt)
	...
	if superseded {
		...
		if _, err := tx.Exec(ctx, `UPDATE deliveries SET state = 'SKIPPED', attempts = $2, updated_at = $3 WHERE id = $1`, request.DeliveryID, attemptNo, now); err != nil {
			return StartResult{}, err
		}
	...
	if state == StateInFlight && lastAttempt != nil && now.Sub(*lastAttempt) < s.config.Lease {
		return StartResult{NotDue: true, RetryAfter: s.config.Lease - now.Sub(*lastAttempt)}, nil
	}
	attemptNo := attempts + 1
	if _, err := tx.Exec(ctx, `UPDATE deliveries SET state = 'IN_FLIGHT', attempts = $2, last_attempt_at = $3, next_attempt_at = NULL, updated_at = $3 WHERE id = $1`, request.DeliveryID, attemptNo, now); err != nil {
...
func (s *PostgresStore) FinishAttempt(ctx context.Context, attempt Attempt, provider ProviderResult, kind FailureKind, deliveryErr error, now time.Time) (FinishResult, error) {
	...
	var current State
	var attempts int
	if err := tx.QueryRow(ctx, `SELECT state, attempts FROM deliveries WHERE id = $1 FOR UPDATE`, attempt.DeliveryID).Scan(&current, &attempts); err != nil {
	...
	if kind == "" {
		...
		if _, err := tx.Exec(ctx, `UPDATE deliveries SET state = 'SUCCEEDED', delivered_at = $2, provider_response = $3, last_error = NULL, updated_at = $2 WHERE id = $1`, attempt.DeliveryID, now, truncate(provider.Response)); err != nil {
			return FinishResult{}, err
		}
		if err := tx.Commit(ctx); err != nil {
	...
	if _, err := tx.Exec(ctx, `UPDATE deliveries SET state = $2, next_attempt_at = $3, last_error = $4, provider_response = $5, updated_at = $6 WHERE id = $1`, attempt.DeliveryID, state, next, errText, truncate(provider.Response), now); err != nil {
		return FinishResult{}, err
	}
	if terminal {
