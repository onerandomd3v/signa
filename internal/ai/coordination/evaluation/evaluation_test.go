...
func TestProcessorRejectsCOD199FactorsNotBoundToEvidence(t *testing.T) {
	tests := []struct {
		...
		{name: "origin independence support for same token", input: evidenceInput([]string{"report one", "report two"}, nil, []string{"opaque-origin-a", "opaque-origin-a"}), mutate: mutateFactor("source_origin", "independence_support", 1, 1), wantError: "source_origin outcome mismatch"},
